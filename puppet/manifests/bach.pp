node default {
  ...
  service { 'bachd':
    ...
    start => 'cd /usr/local/src/bach && nohup bundle exec /usr/local/src/bach/bin/bachd >/tmp/bachd.log 2>/tmp/bachd.err &',
    require => [ Service['broker'], Exec['bundle gems'], Package['espeak'] ]
  }
}
